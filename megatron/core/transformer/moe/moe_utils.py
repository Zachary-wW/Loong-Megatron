...
# ============================================================================
# MoE memory / token monitoring (migrated from AIAK, M-05-2).
# Community MoE code has no memory monitoring; these utilities track max CUDA
# memory usage and peak dispatcher tokens per rank and dump them to files.
# ============================================================================
...
def write_monitor_data_to_file(monitor_dir, print_mem_monitor_interval):
    ...
    if max_dispatcher_tokens > MEM_MONITOR_FORCE_PRINT_TOKEN_THRESHOLD:
        print(f"WARNING: max_dispatcher_tokens({max_dispatcher_tokens}) "
              f"exceeds threshold of {MEM_MONITOR_FORCE_PRINT_TOKEN_THRESHOLD}. Printing anyway.")
    ...
    if force_print or data["index"] % print_mem_monitor_interval == 0:
        ...
        with open(os.path.join(monitor_dir, f"{rank}_temp"), "w", encoding="utf-8") as f:
            f.write(f"rank: {rank}|t: {data['tp_rank']}|p: {data['pp_rank']}|e: {data.get('ep_rank')}")
