...
class MultiLatentAttention(Attention):
    ...
    def __init__(
        ...
        # Only the DSA core attention accepts is_mtp_layer; other core_attention
        # impls (TE DotProductAttention, etc.) would reject the kwarg.
    ...
    def check_kv_cache_grad_consumed(self):
        """MLA-specific override for ChunkPipe SFT grad-consumption check.

        Standard SelfAttention tracks per-chunk grads in ``key_cache_grad`` /
        ``value_cache_grad``; MLA stores them in ``kv_compressed_cache_grad`` /
        ``key_pos_emb_cache_grad`` (see ``init_chunk_key_value_cache_mla``).
        Any leftover entries after a group's backward indicate a LIFO order
        violation, mirroring the base-class semantics.
        """
