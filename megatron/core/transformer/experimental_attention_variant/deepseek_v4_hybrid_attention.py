...
class off_interface:
    """Compat shim for FineGrainedActivationOffloadingInterface."""
    def __init__(self, offload, tensor, name):
        self.offload = offload
        self.name = name
        if offload:
            self.tensor = fine_grained_offloading_group_start(tensor, name)
        else:
            self.tensor = tensor
    def __enter__(self):
        return self
    def __exit__(self, *args):
        pass
    def group_offload(self, tensor, **kwargs):
        """fine_grained_offloading_group_commit"""
        if self.offload:
...
# LayerNormBuilder not available; use type alias
LayerNormBuilder = type
from megatron.core.transformer.transformer_config import MLATransformerConfig
# typed_torch not available; apply_module is identity
def apply_module(m):
    """m""" 
    return m
