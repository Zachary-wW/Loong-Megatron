...
class Platform:
    ...
    def __init__(self) -> None:
        ...
        force_generic = os.environ.get("LCE_FORCE_GENERIC", "0") == "1"

        if cc[0] == 10 and not force_generic:
            try:
                ...
                logger.info(
                    f"LinearCrossEntropy: Blackwell GPU detected (cc={cc}) but CUTLASS kernels "
                    f"not available. Falling back to generic (pure PyTorch) path."
                )
                from .linear_cross_entropy.generic import entry as generic_entry

                self.forward_func = generic_entry.forward
                self.backward_func = generic_entry.backward
        else:
            if force_generic:
                logger.info(
                    "LinearCrossEntropy: LCE_FORCE_GENERIC=1, using generic (pure PyTorch) path."
                )
            else:
                logger.info(
                    f"LinearCrossEntropy: no optimized kernel for compute capability {cc}. "
                    f"Using generic (pure PyTorch) fallback path."
                )
            from .linear_cross_entropy.generic import entry as generic_entry
