...
"""
Blackwell (SM100) CUTLASS/CuTe backward kernel for fused linear cross-entropy.

This module implements ``BwdPartialDlogits``, which computes the partial gradient
w.r.t. logits (``d_logits``) for one vocabulary split during the backward pass of
fused linear cross-entropy.

Algorithm
---------
Given the softmax statistics saved in the forward pass (``maximum``, ``accumulate``),
the kernel re-computes the logits for the current vocab split
(``hidden @ weight_chunk^T``) and derives ``d_logits`` in the epilogue:

    d_logits[t, v] = softmax_prob[t, v] * (d_loss / accumulate[t])
                     - (v == label[t]) * d_loss

The result is written to a caller-supplied output buffer of shape
``(num_tokens, vocab_per_split)``, which the host loop then uses to accumulate
``d_hidden`` and ``d_weight_chunk`` via cuBLAS.

Hardware utilisation (Blackwell-specific)
-----------------------------------------
* **UMMA** — Blackwell tensor-core MMA instruction (SM100 ``tcgen05``), with a
  configurable ``mma_tiler`` (default 128×256).
* **TMEM** — Tensor Memory accumulator for the MMA output; logits are kept in
  TMEM through the epilogue and never written to global memory.
* **TMA** — Asynchronous bulk data loads for operands A (hidden) and B (weight).
* **Multi-stage pipeline** — 4-stage A/B prefetch pipeline + 1-stage TMEM
  accumulator pipeline to overlap compute and data movement.
* **2-CTA cluster mode** — Optional ``use_2cta_instrs`` flag enables SM100 2-CTA
  cooperative MMA for higher throughput on large tiles.

Warp layout per CTA (8 warps × 32 threads)
-------------------------------------------
* Warps 0-3 : epilogue (read TMEM, compute softmax grad, write d_logits)
* Warp  4   : TMA load
* Warp  5   : MMA
...
Entry point
-----------
The public API is ``BwdPartialDlogits.__call__``, invoked by ``entry.py`` inside
the per-split backward loop.
"""
...
try:
    ...
    class BwdPartialDlogits:
        ...
        def _compute_grid(
            ...
            """
            Compute the grid dimensions for the kernel launch.

            Args:
                problem_mnk: Tuple of (M, N, K) dimensions.
                cluster_shape_mn: Cluster shape in (M, N) dimensions.
                cta_tiler: CTA tile shape in (M, N, K) dimensions.

            Returns:
                Grid dimensions as (grid_x, grid_y, grid_z).
            """
        ...
        def _compute_stages(
            ...
            """
            Compute the number of pipeline stages for the kernel.

            Args:
                tiled_mma: The tiled MMA operation.
                mma_tiler: The MMA tile shape.
                a_dtype: Data type for operand A.
                b_dtype: Data type for operand B.

            Returns:
                Tuple of (num_acc_stage, num_ab_stage, num_epi_stage_per_tile).
            """
            num_acc_stage = 1
            num_ab_stage = 4
        ...
        def _setup_attributes(
            ...
            """
            Setup the kernel attributes based on the MMA configuration.
        ...
        def __call__(
            ...
            """
            Launch the backward kernel for partial d_logits computation.

            Args:
                split_idx: Index of the vocabulary split.
                hidden: Input hidden states tensor.
                weight: Embedding weight tensor.
                labels: Labels tensor.
                dlogprobs: Gradient of log probabilities.
                maximum: Maximum values tensor.
                accu: Accumulated values tensor.
                dlogits_partial: Output partial d_logits tensor.
                scalarNumValidTokens: Pointer to the number of valid tokens.
                ignore_index: Index to ignore in labels.
                rank: Tensor parallelism rank.
